...
impl U256 {
    ...
    fn unbounded_shl(&self, rhs: u32) -> U256 {
        if rhs < 128 {
            ...
            let upper = self.0[0].unbounded_shl(rhs - 128);
            U256([0, upper])
        }
    }
...
impl Byte {
    ...
    pub(crate) fn insert(&mut self, val: u8) {
        // TODO: Figure out semantics regarding Uninit bytes.
        let b = Byte::from_iters([val..=val]);
        let v = self.values.as_mut().unwrap();
        *v = v.or(&b.values.unwrap());
    }
    ...
    pub(crate) fn iter(&self) -> impl Iterator<Item = Option<u8>> {
        let mut state = Some(self.values);
        std::iter::from_fn(move || match state {
            ...
            Some(Some(v)) => {
                let next_one = v.trailing_zeros();
                if next_one == 256 {
                    state = None;
                    None
                } else {
                    let mask = U256([1, 0]).unbounded_shl(next_one).not();
                    state = Some(Some(v.and(&mask)));
                    Some(Some(next_one.try_into().unwrap()))
                }
            }
        })
    }
